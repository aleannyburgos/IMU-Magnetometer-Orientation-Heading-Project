...
/* we need a reference for our level and the current values for pitch, roll and yaw. From there we find the difference
 ...
 * float yaw =0.0f;
 *
 * float diff_pitch = pitch_ref - pitch;
 * float diff_yaw = yaw_ref - yaw;
 * float diff_roll = roll_ref - roll;
 *
 *
 * gains for fans you tune experimentally
 * float Kp_roll = ..., Kd_roll = ...;
 * float Kp_pitch = ..., Kd_pitch = ...;
 * desired torques around X and Y
 * float tau_roll  = Kp_roll  * e_roll  - Kd_roll  * wx;
 * float tau_pitch = Kp_pitch * e_pitch - Kd_pitch * wy;
 * float k_pwm = ...;   // N per PWM unit (you can treat it as tuning)
 * pwm_L = pwm_base + k_pwm * ( tau_pitch / (2.0f * L) );
 * pwm_R = pwm_base - k_pwm * ( tau_pitch / (2.0f * L) );
 */
